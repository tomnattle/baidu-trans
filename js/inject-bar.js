...
function injectBar(from, to, langmap) {
  if ($("#translate-head").length <= 0) {
    ...
    var head_bar = $("<div>", {
      ...
      style:
        "background-color:#f0f0f0 !important;z-index: 99999 !important;font-size:12px !important;color:#333 !important",
    }).css({
      position: "fixed",
      width: "30%",
      //   height: "36px",
    ...
    function createFromElement() {
      var selectFromBox = $("<div>")
        .attr({
          id: "translate-head-from",
          class: "translate-head-retranslate",
          style:
            "line-height:23px !important;cursor:pointer !important; display:none",
          "data-click": "no-click",
        })
        .css({
          width: "97px",
      ...
      var selectFromElement = $("<span>")
        .attr({
          id: "detect-from",
          value: from,
          /* eslint-disable max-len */
          style:
            "cursor:pointer !important;font-size:12px !important;white-space:nowrap !important;color:#333 !important",
        })
        .css({
          position: "relative",
          background: "initial",
      ...
      for (key in langmap) {
        selectFromInnerBox.append(
          $("<div>")
            .attr({
              /* globals bridge */
              value: key,
              style:
                "cursor:pointer !important;line-height:25px !important;font-size:12px !important;color:#333 !important",
            })
            .html(langmap[key])
            .css({
              "box-sizing": "border-box",
            ...
            .on("click", function (e) {
              ...
              $("#select-inner-from").children().attr({
                style:
                  "cursor: pointer !important; line-height: 25px !important; font-size: 12px !important; color: rgb(51, 51, 51) !important; box-sizing: border-box; padding-left: 11px; height: 25px; background-color: rgb(255, 255, 255);",
              });
              $(this).attr({
                style:
                  "cursor: pointer !important; line-height: 25px !important; font-size: 12px !important; color: rgb(67, 149, 255) !important; box-sizing: border-box; padding-left: 11px; height: 25px; background-color: rgb(255, 255, 255);",
              });
    ...
    function createToElement() {
      ...
      var selectToElement = $("<span>")
        .attr({
          id: "detect-to",
          value: to,
          style:
            "cursor:pointer !important;font-size:12px !important;color:#333 !important",
        })
        .css({
          position: "relative",
          background: "initial",
    ...
    // 翻译按钮
    var tranlateButtonElement = $("<div>")
      .attr({
        id: "src-reload",
        style:
          "cursor: pointer !important;color: white !important;text-align:center !important;" +
          "border-radius: 3px !important;font-size: 12px !important;line-height:23px !important",
      })
      .css({
        position: "fixed",
        right: "30px",
    ...
    $("#translate-head-from #select-inner-from")
      .children()
      .each(function (index, el) {
        if (el.getAttribute("value") === "auto") {
        } else if (
          el.getAttribute("value") ===
          $("#translate-head-from #detect-from").attr("value")
        ) {
          el.setAttribute(
            "style",
            "cursor: pointer !important; line-height: 25px !important; font-size: 12px !important; color: rgb(67, 149, 255) !important; box-sizing: border-box; padding-left: 11px; height: 25px; background-color: rgb(255, 255, 255);"
    ...
    $("#translate-head-to #select-inner-to")
      .children()
      .each(function (index, el) {
        if (
          el.getAttribute("value") ===
          $("#translate-head-to #detect-to").attr("value")
        ) {
          el.setAttribute(
            "style",
            "cursor: pointer !important; line-height: 25px !important; font-size: 12px !important; color: rgb(67, 149, 255) !important; box-sizing: border-box; padding-left: 11px; height: 25px; background-color: rgb(255, 255, 255);"
    ...
    $("#translate-head-cross").click(function () {
      ...
      $("body").attr(
        "style",
        "position: relative; margin-top: " + body_margin_top + "px !important;"
      );
      recoverFixNodes();
      $("trans").each(function () {
        //$(this).text($(this).attr("data-src"));
        if ($(this).attr("data-src")) {
          $(this).html($(this).attr("data-src") + "<br>" + $(this).text());
        }
      });
      $("#src-reload").text("重新翻译");
    });
    $("#translate-head-cross").hover(
      function () {
        /* globals bridge */
        $("#translate-head-cross-img").attr(
          "src",
          chrome.extension.getURL("imgs/map/close_hover.png")
        );
      },
      function () {
        /* globals bridge */
        $("#translate-head-cross-img").attr(
          "src",
          chrome.extension.getURL("imgs/map/close.png")
        );
      }
    );
    ...
    $("#src-reload").click(function () {
      ...
      if ($(this).text() === "显示原文") {
        ...
        $("trans").each(function () {
          if ($(this).attr("data-src")) {
            $(this).html(
              `<span style="opacity: 0.7;">${$(this).attr(
                "data-src"
              )}</span></br><span>${$(this).attr("data-dst")}</span>`
            );
            //$(this).html($(this).attr("data-src") + "<br>" + $(this).text());
          }
        });
    ...
    $("#fanyi_baidu_com").hover(
      function () {
        $(this).attr(
          "style",
          "float:right;margin-right:15px;margin-top:11px;text-decoration: none;color:#4395FF !important;font-size:12px !important"
        );
      },
      function () {
        $(this).attr(
          "style",
          "float:right;margin-right:15px;margin-top:11px;text-decoration: none;font-size:12px !important;color:#333 !important"
        );
      }
    );

    $("#tousu_baidu_com_fanyi_add").hover(
      function () {
        $(this).attr(
          "style",
          "float:right;margin-right:15px;margin-top:11px;text-decoration: none;color:#4395FF !important;font-size:12px !important"
