...
function walkParserDom(handler_dom) {
  for (var i in handler_dom) {
    if (
      handler_dom[i].type === "tag" &&
      noTranslateTagRe.test(handler_dom[i].name)
    ) {
      continue;
    }
  ...
  if (node !== null) {
    while (node != null) {
      ...
      if (node.nodeType === 3) {
        ...
        trans.innerHTML = `<span style="opacity: 0.7;">${node.nodeValue}</span></br><span>${dst}</span>`;
...
function translate(from, to) {
  ...
  temp_list = [];
  /* globals bridge */
  var handler = new Tautologistics.NodeHtmlParser.DefaultHandler(function (
    error,
    dom
  ) {
    if (error) {
      ...
      if (from === "auto") {
        if (/www.gov.ro|mfa.bg/i.test(window.location.href)) {
          if (/www.gov.ro/i.test(window.location.href)) {
            $("#translate-head-from #detect-from")
              .html("检测到" + "罗马尼亚语")
              .attr("value", "rom");
          ...
          /* eslint-disable max-len */
          $("#translate-head-from #detect-from")
            .html("检测到" + langmap[response.src_dir])
            .attr("value", response.src_dir);
...
function langDetect() {
  ...
  var buttonTranslateElement = $("<div>")
    .attr({
      id: "detect-translate",
      /* eslint-disable max-len */
      style:
        "cursor:pointer !important;border-radius:3px !important;color:white !important;" +
        "font-size:12px !important;text-align:center !important;line-height:23px !important",
    })
    .css({
      padding: "initial",
      position: "fixed",
  ...
  var buttonDetectButton = $("<div>")
    .attr({
      id: "detect-no-translate",
      /* eslint-disable max-len */
      style:
        "cursor:pointer !important;border-radius:3px !important;cursor:pointer !important;" +
        "font-size:12px !important;color:#333 !important;text-align:center !important;line-height:23px !important",
    })
    .css({
      padding: "initial",
      position: "fixed",
  ...
  var aTipsElement = $("<a>")
    ...
    .hover(
      function () {
        $(this).attr(
          "style",
          "position: fixed;left:585px;top:12px;text-decoration: none;color:#4395FF !important;font-size:12px !important"
        );
      },
      function () {
        $(this).attr(
          "style",
          "position: fixed;left:585px;top:12px;text-decoration: none;font-size:12px !important;color:#333 !important"
        );
      }
    );
  ...
  $("#select-translate-from #select-inner-box")
    .children()
    .each(function (index, el) {
